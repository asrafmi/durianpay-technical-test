...
<template>
    <div class="flex flex-col gap-5.5">
        ...
        <div class="flex">
            ...
            <div v-if="authStore.isOperation" class="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-4 w-full">
                <template v-if="paymentStore.isLoadingPaymentSummary">
                    <SummaryCardSkeleton v-for="i in 4" :key="i" />
                </template>
                <template v-else>
                    <SummaryCard label="Total payments" :value="total" :percentage="0" showMultiBar :bars="[
                        { color: '#1A9E5C', percentage: pct(completed) },
                        { color: '#2563EB', percentage: pct(processing) },
                        { color: '#E31C4D', percentage: pct(failed) },
                    ]" :statusFilter="StatusFilter.ALL" @select="handleStatusChange" />
        ...
        <div class="flex flex-col gap-2.5 sm:flex-row sm:flex-wrap sm:items-center">
            <SearchInputWithHistory v-model="searchQuery" id="search-field" placeholder="Search merchant or payment ID…"
                containerClass="w-full sm:min-w-[220px] sm:flex-1" />
            <NumberInput v-model="minimumAmount" id="minimum-amount" placeholder="Minimum amount"
                containerClass="w-full sm:min-w-[220px] sm:flex-1" />
            ...
            <div class="relative flex gap-0.5 overflow-x-auto rounded-[9px] bg-bg-light p-0.75">
                ...
                <button v-for="item in statusFilters" :key="item.value" ref="filterItems" type="button"
        ...
        <PaymentTable v-else :rows="rows" :formatDate="formatDate" :formatAmount="formatAmount" :sort="sort"
            @sort-toggle="handleSortToggle" @view-detail="handleViewDetail" />

        <Pagination :rows="rows" :total="paymentStore.total" :currentPage="currentPage" :pageSize="pageSize"
            :goToPage="goToPage" :goToPrevPage="goToPrevPage" :goToNextPage="goToNextPage"
            :changePageSize="changePageSize" />
    </div>

    <PaymentDetailPanel :payment="selectedPayment" :isOpen="isPanelOpen" @close="handleClosePanel"
